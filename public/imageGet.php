...
function ciniki_patents_imageGet($ciniki) {
    ...
    $rc = ciniki_patents_checkAccess($ciniki, $args['tnid'], 'ciniki.patents.imageGet');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    //
        ...
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.patents.10', 'msg'=>'Image not found', 'err'=>$rc['err']));
        }
        if( !isset($rc['image']) ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.patents.11', 'msg'=>'Unable to find Image'));
        }
        $image = $rc['image'];
    }
